<div class="row my-2" data-row='{{ "$uuid-$index" }}' data-row-index="{{ $index }}" data-repeat="{{ $uuid }}"
    data-locale='{{ $locale }}'>
    <label for="">{{ $label }}</label>
    <div class='col-10'>
        <div class='form-group'>
            <input type="hidden" name='{{ $name . "[$index]" . '[id]' }}' value="{{ $model?->id }}">
            <input {!! $attributes ??= "required  type='text'" !!} name='{{ $name . "[$index]" . "[$name:$locale]" }}'
                id='{{ $name . "[$index]" . "[$name:$locale]" }}'
                value="{{ old($name . "[$index]" . "[$name:$locale]", $model?->translate($locale)?->$name) }}"
                class='form-control'>
